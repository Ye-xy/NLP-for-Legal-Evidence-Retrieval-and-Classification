...
Original file is located at
    https://colab.research.google.com/drive/1QXDk88a70qZWd25KR1Sx3vdPPxaNI96L
...
"""## Setup for FAISS Retriever Methods
- `retrieve_faiss()`: Retrieves top-k semantically similar evidence using dense vector search (cosine similarity).

This section uses SentenceTransformer embeddings (`MiniLM-L12-v2`) and FAISS with inner product (normalized for cosine similarity).
...
This section evaluates the RNN classifier trained on FAISS-retrieved evidence. It includes:
...
print("FAISS-L12 Retriever")

# Retrieve FAISS evidences
faiss_train_retrieved = {cid: retrieve_faiss(info["claim_text"], top_k=5) for cid, info in train_claims_strat.items()}
faiss_dev_retrieved   = {cid: retrieve_faiss(info["claim_text"], top_k=5) for cid, info in dev_claims_strat.items()}

# Inject gold evidence (during training only)
inject_gold_evidence(train_claims_strat, faiss_train_retrieved, top_k=5)

# Build vocab from training data
vocab_faiss = build_vocab_with_cache(train_claims_strat, evidence_dict, faiss_train_retrieved)

# Build datasets
train_dataset = RNNDatasetCached(train_claims_strat, evidence_dict, faiss_train_retrieved, vocab_faiss)
dev_dataset   = RNNDatasetCached(dev_claims_strat, evidence_dict, faiss_dev_retrieved, vocab_faiss)
...
# Initialize model
model = RNNClassifier(len(vocab_faiss), 128, 128, 4)
model_path = "rnn_faiss_l12.pt"
...
model = RNNClassifier(len(vocab_faiss), 128, 128, 4)
model.load_state_dict(torch.load("rnn_faiss_l12.pt", map_location=device))
model.to(device)
model.eval()
...
# Re-run FAISS retrieval over full dev set
faiss_dev_retrieved_full = {
    cid: retrieve_faiss(info["claim_text"], top_k=5) for cid, info in dev_claims.items()
}

# Build full dev dataset using new retrieval
full_dev_dataset = RNNDatasetCached(dev_claims, evidence_dict, faiss_dev_retrieved_full, vocab_faiss)
...
with torch.no_grad():
    for batch in tqdm(full_dev_loader, desc="Generating predictions"):
        ...
        for cid, pred in zip(claim_ids, preds.cpu().numpy()):
            expanded_evidences = expand_ids(faiss_dev_retrieved_full[cid], canonical_to_all_ids, top_k=5)
            predictions[cid] = {
...
# Save to file
with open("dev-claims-rnn-faiss.json", "w") as f:
    json.dump(predictions, f, indent=2)

!python3 eval.py --predictions dev-claims-rnn-faiss.json --groundtruth dev-claims.json
