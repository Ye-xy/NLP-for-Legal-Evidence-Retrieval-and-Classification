# -*- coding: utf-8 -*-
"""03_LSTM_Hybrid.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1N0Y4vmznT_jKOUL5Do5eh-cOLuCnIyrU
...
class LSTMWithAttention(nn.Module):
    def __init__(self, vocab_size, embed_size=128, hidden_size=128, num_classes=4, embedding_weights=None):
...
# Dataset
train_dataset = LSTMDatasetCached(train_claims, evidence_dict, precomputed_train, vocab_hybrid)
dev_dataset   = LSTMDatasetCached(dev_claims, evidence_dict, precomputed_dev, vocab_hybrid)
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), "lstm_hybrid_best_model.pt")
...
model.load_state_dict(torch.load("lstm_hybrid_best_model.pt", map_location=device))
model.to(device)
model.eval()
...
with open("dev-claims-lstm-hybrid.json", "w") as f:
    json.dump(predictions, f, indent=2)

!python3 eval.py --predictions dev-claims-lstm-hybrid.json --groundtruth dev-claims.json
