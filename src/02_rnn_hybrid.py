...
Original file is located at
    https://colab.research.google.com/drive/1bUScOgeH7cv-VNVFmBiO9wyvwqHlDoeG
...
This section evaluates the RNN classifier trained on hybrid-retrieved evidence. It includes:
...
"""## Hybrid (TF-IDF + FAISS) Retriever Evaluation
...
print("Hybrid Retriever [TF-IDF + FAISS]")

# Build hybrid retriever
# Best alpha value obtained from 01_Exoloratory file experiments
retrieve_hybrid = make_hybrid_retriever(0.2, vectorizer, evidence_tfidf, faiss_index, embedder, evidence_ids)

# Retrieve evidence for training/dev claims
hybrid_train_retrieved = {
    cid: retrieve_hybrid(info["claim_text"]) for cid, info in train_claims_strat.items()
}
hybrid_dev_retrieved = {
    cid: retrieve_hybrid(info["claim_text"]) for cid, info in dev_claims_strat.items()
}

# Inject gold evidence (during training only)
inject_gold_evidence(train_claims_strat, hybrid_train_retrieved, top_k=5)

# Build vocabulary based on training set + retrieved evidence
vocab_hybrid = build_vocab_with_cache(train_claims_strat, evidence_dict, hybrid_train_retrieved)

# Create train and dev datasets using the cached evidence and vocab
train_dataset = RNNDatasetCached(train_claims_strat, evidence_dict, hybrid_train_retrieved, vocab_hybrid)
dev_dataset   = RNNDatasetCached(dev_claims_strat, evidence_dict, hybrid_dev_retrieved, vocab_hybrid)

# Wrap datasets in DataLoaders for batch training
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
dev_loader   = DataLoader(dev_dataset, batch_size=32)

# Initialize and train the RNN classifier using hybrid-retrieved evidence
model = RNNClassifier(len(vocab_hybrid), 128, 128, 4)
model_path = "rnn_hybrid.pt"
...
model = RNNClassifier(len(vocab_hybrid), 128, 128, 4)
model.load_state_dict(torch.load("rnn_hybrid.pt", map_location=device))
model.to(device)
model.eval()
...
# Re-run hybrid retrieval over full dev set
hybrid_dev_retrieved_full = {
    cid: retrieve_hybrid(info["claim_text"], top_k=5) for cid, info in dev_claims.items()
}

# Build dev dataset using new retrieval
full_dev_dataset = RNNDatasetCached(dev_claims, evidence_dict, hybrid_dev_retrieved_full, vocab_hybrid)
...
with torch.no_grad():
    for batch in tqdm(full_dev_loader, desc="Generating predictions"):
        ...
        for cid, pred in zip(claim_ids, preds.cpu().numpy()):
            expanded_evidences = expand_ids(hybrid_dev_retrieved_full[cid], canonical_to_all_ids, top_k=5)
            predictions[cid] = {
...
# Save output
with open("dev-claims-rnn-hybrid.json", "w") as f:
    json.dump(predictions, f, indent=2)

!python3 eval.py --predictions dev-claims-rnn-hybrid.json --groundtruth dev-claims.json
