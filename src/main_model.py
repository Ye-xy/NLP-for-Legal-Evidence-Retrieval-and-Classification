...
Original file is located at
    https://colab.research.google.com/drive/1uDVyCYoAMXJk60A-6r30nHvxZkzVWaEI
...
retrieve_hybrid = make_hybrid_retriever(
    0.2,
    vectorizer,
    evidence_tfidf,
    faiss_index,
...
hybrid_train_retrieved = {
    cid: retrieve_hybrid(info["claim_text"], top_k=5)
    for cid, info in train_claims.items()
}

inject_gold_evidence(train_claims, hybrid_train_retrieved, top_k=5)

precomputed_train = {
    cid: {
        "claim_text": train_claims[cid]["claim_text"],
        "evidence_ids": hybrid_train_retrieved[cid]
    }
...
precomputed_dev = {
    cid: {
        ...
        "evidence_ids": retrieve_hybrid(info["claim_text"], top_k=5)
    }
...
# vocab
vocab_hybrid = build_vocab_with_cache(train_claims, evidence_dict, precomputed_train)

# Dataset
train_dataset = GRUDatasetCached(train_claims, evidence_dict, precomputed_train, vocab_hybrid)
dev_dataset   = GRUDatasetCached(dev_claims, evidence_dict, precomputed_dev, vocab_hybrid)
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), "gru_hybrid_best_model.pt")
...
model.load_state_dict(torch.load("gru_hybrid_best_model.pt", map_location=device))
model.to(device)
model.eval()
...
with open("dev-claims-gru-hybrid.json", "w") as f:
    json.dump(predictions, f, indent=2)

!python3 eval.py --predictions dev-claims-gru-hybrid.json --groundtruth dev-claims.json
