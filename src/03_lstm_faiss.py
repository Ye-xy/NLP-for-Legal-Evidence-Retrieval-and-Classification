...
Original file is located at
    https://colab.research.google.com/drive/1LZPzna_-MpEyV3YiB9-0mDB_adDrxnT1
...
# Dataset
train_dataset = LSTMDatasetCached(train_claims, evidence_dict, precomputed_train, vocab_faiss)
dev_dataset   = LSTMDatasetCached(dev_claims, evidence_dict, precomputed_dev, vocab_faiss)
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), "lstm_faiss_best_model.pt")
...
model.load_state_dict(torch.load("lstm_faiss_best_model.pt", map_location=device))
model.to(device)
model.eval()
...
with open("dev-claims-lstm-faiss.json", "w") as f:
    json.dump(predictions, f, indent=2)

!python3 eval.py --predictions dev-claims-lstm-faiss.json --groundtruth dev-claims.json
